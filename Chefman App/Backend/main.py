...
if OPENAI_AVAILABLE:
    ...
    if openai_api_key:
        try:
            ...
            print("⚠️ Recipe analysis features will be disabled")
            openai_client = None
    else:
...
def find_recipe_document(recipe_id: int):
    ...
    query = recipes_ref.where('id', '==', recipe_id).limit(1).get()
    if query:
        return query[0]
    
    # Method 3: Try to find by document ID that matches the integer or hash (limit to 1000 for performance)
    all_docs = recipes_ref.limit(1000).get()
    for doc in all_docs:
        doc_id = doc.id
...
def _stable_hash(s):
    """Generate a stable hash for a string (consistent across sessions)"""
...
def get_recipe_counts(recipe_id: int) -> dict:
    ...
    # Count likes (use .get() instead of .stream() for better performance)
    likes_query = likes_ref.where('recipe_id', '==', recipe_id).get()
    likes_count = len(likes_query)
    
    # Count comments
    comments_query = comments_ref.where('recipe_id', '==', recipe_id).get()
    comments_count = len(comments_query)
    
    # Count favorites
    favorites_query = favorites_ref.where('recipe_id', '==', recipe_id).get()
    favorites_count = len(favorites_query)
    
    # Count completed (todo items with completed=True)
    completed_query = todos_ref.where('recipe_id', '==', recipe_id).where('completed', '==', True).get()
    todo_count = len(completed_query)
...
def get_user(user_id: int):
    """Get user by ID - supports multiple lookup methods"""
    users_ref = db.collection('users')
    ...
    if user_doc.exists:
        ...
        if user_data:
            # Convert document ID to integer if needed
            doc_id = user_doc.id
            if doc_id.isdigit():
                user_data['id'] = int(doc_id)
            else:
                user_data['id'] = abs(hash(doc_id)) % (10**9)
            user_response_data = {k: v for k, v in user_data.items() if k not in ['password_hash', 'password']}
            return UserResponse(**user_response_data)
    
    # Method 2: Try to find by querying the 'id' field
    query = users_ref.where('id', '==', user_id).limit(1).get()
    if query:
        user_doc = query[0]
        user_data = doc_to_dict(user_doc)
        if user_data:
    ...
    # This handles cases where user_id is a hash of the document ID
    docs = users_ref.limit(1000).get()
    for doc in docs:
        doc_id = doc.id
        # Check if document ID matches user_id
        if doc_id.isdigit() and int(doc_id) == user_id:
            user_data = doc_to_dict(doc)
            if user_data:
                user_data['id'] = user_id
                user_response_data = {k: v for k, v in user_data.items() if k not in ['password_hash', 'password']}
                return UserResponse(**user_response_data)
        # Check if stable hash of document ID matches user_id
        elif _stable_hash(doc_id) == user_id:
            user_data = doc_to_dict(doc)
            if user_data:
                user_data['id'] = user_id
                user_response_data = {k: v for k, v in user_data.items() if k not in ['password_hash', 'password']}
...
def get_user_profile(user_id: int, current_user: dict = Depends(get_current_user)):
    """Get user profile with statistics - supports multiple lookup methods"""
    users_ref = db.collection('users')
    user_doc = None
    user_data = None
    ...
    recipes_ref = db.collection('recipes')
    user_recipes_query = recipes_ref.where('creator_id', '==', user_id).get()
    recipes_count = len(user_recipes_query)
    
    # Get followers count
    follows_ref = db.collection('follows')
    followers_query = follows_ref.where('following_id', '==', user_id).get()
    followers_count = len(followers_query)
    
    # Get following count
    following_query = follows_ref.where('follower_id', '==', user_id).get()
    ...
    if recipe_ids:
        ...
        for recipe_id in recipe_ids:
            recipe_likes = likes_ref.where('recipe_id', '==', recipe_id).get()
            total_likes += len(recipe_likes)
    
    # Ensure id is an integer
    user_id_value = user_data.get('id', user_id)
    ...
    response_data = {
        "id": int(user_id_value),  # Ensure it's an integer
        "username": str(user_data.get('username', 'Unknown')),  # Ensure string
        "recipes_count": int(recipes_count),  # Ensure integer
        "followers_count": int(followers_count),  # Ensure integer
        "following_count": int(following_count),  # Ensure integer
        "total_likes_received": int(total_likes)  # Ensure integer
    }
    
    # Add optional fields (only if they exist and are not None)
    email = user_data.get('email')
    if email is not None:
        response_data["email"] = str(email)
    else:
        response_data["email"] = None
    
    bio = user_data.get('bio')
    if bio is not None:
        response_data["bio"] = str(bio)
    else:
        response_data["bio"] = None
    
    gender = user_data.get('gender')
    if gender is not None:
        response_data["gender"] = str(gender)
    else:
        response_data["gender"] = None
    
    profile_image_url = user_data.get('profile_image_url')
    if profile_image_url is not None:
        response_data["profile_image_url"] = str(profile_image_url)
    else:
...
def get_users():
    ...
    for doc in docs:
        user_data = doc_to_dict(doc)
        if user_data:
            # Convert document ID to integer if it's a string
            doc_id = doc.id
            if isinstance(user_data.get('id'), str) or not isinstance(user_data.get('id'), int):
                if doc_id.isdigit():
                    user_data['id'] = int(doc_id)
                else:
                    # Use hash of document ID to generate integer ID
                    user_data['id'] = abs(hash(doc_id)) % (10**9)
            user_data = {k: v for k, v in user_data.items() if k not in ['password_hash', 'password']}
            try:
                users.append(UserResponse(**user_data))
            except Exception as e:
                print(f"⚠️ Error creating UserResponse for user {doc_id}: {e}")
                continue
    return users
...
def read_recipes(skip: int = 0, limit: int = 100):
    ...
    try:
        ...
        max_limit = min(limit + skip, 1000)  # Limit to 1000 for performance
        query = recipes_ref.limit(max_limit).get()
...
def get_todo_list(current_user: dict = Depends(get_current_user)):
    ...
    todos_ref = db.collection('todo_list')
    todos_query = todos_ref.where('user_id', '==', current_user['id']).get()
...
def get_user_interactions(recipe_id: int, current_user: dict = Depends(get_current_user)):
    """Get user's interactions with a recipe - optimized to use .get() instead of .stream()"""
    user_id = current_user['id']
    
    # Check favorite - use .get() instead of .stream() for better performance
    favorites_ref = db.collection('favorites')
    favorite_query = favorites_ref.where('user_id', '==', user_id).where('recipe_id', '==', recipe_id).limit(1).get()
    is_favorited = len(favorite_query) > 0
    
    # Check like - use .get() instead of .stream() for better performance
    likes_ref = db.collection('likes')
    like_query = likes_ref.where('user_id', '==', user_id).where('recipe_id', '==', recipe_id).limit(1).get()
    is_liked = len(like_query) > 0
    
    # Check todo - use .get() instead of .stream() for better performance
    todos_ref = db.collection('todo_list')
    todo_query = todos_ref.where('user_id', '==', user_id).where('recipe_id', '==', recipe_id).limit(1).get()
    is_in_todo = len(todo_query) > 0
    ...
    if recipe_doc and recipe_doc.exists:
        ...
        if creator_id and creator_id != user_id:
            follows_ref = db.collection('follows')
            # Use .get() instead of .stream() for better performance
            follow_query = follows_ref.where('follower_id', '==', user_id).where('following_id', '==', creator_id).limit(1).get()
            is_following_creator = len(follow_query) > 0
...
def get_follow_status(user_id: int, current_user: dict = Depends(get_current_user)):
    """Get follow status between current user and target user - optimized to use .get() instead of .stream()"""
    follower_id = current_user['id']
    
    follows_ref = db.collection('follows')
    # Use .get() instead of .stream() for better performance
    follow_query = follows_ref.where('follower_id', '==', follower_id).where('following_id', '==', user_id).limit(1).get()
    is_following = len(follow_query) > 0
    
    return {"is_following": is_following}

@app.get("/users/me/stats")
def get_user_stats(current_user: dict = Depends(get_current_user)):
    """Get current user's statistics - uses same logic as /users/{user_id}/profile"""
    user_id = current_user['id']
    
    # Get followers count (how many users follow this user)
    # Use .get() instead of .stream() for consistency with /users/{user_id}/profile
    follows_ref = db.collection('follows')
    followers_query = follows_ref.where('following_id', '==', user_id).get()
    followers_count = len(followers_query)
    
    # Get following count (how many users this user follows)
    # Use .get() instead of .stream() for consistency
    following_query = follows_ref.where('follower_id', '==', user_id).get()
    following_count = len(following_query)
    ...
    # Get total likes received (same logic as /users/{user_id}/profile)
    recipe_ids = []
    for recipe_doc in user_recipes_query:
        doc_id = recipe_doc.id
        if doc_id.isdigit():
